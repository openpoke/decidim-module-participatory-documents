...
class CreateDecidimParticipatoryDocumentsSuggestions < ActiveRecord::Migration[6.0]
  def change
    create_table :decidim_participatory_documents_suggestions do |t|
      ...
      t.references :suggestable, null: false, polymorphic: true, index: { name: "decidim_pd_suggesstable" }
      t.references :decidim_author, polymorphic: true, null: false, index: { name: "decidim_pd_suggestions_author" }
